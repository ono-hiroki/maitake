# =============================================================================
# 04-cloudsql: Cloud SQL (PostgreSQL) / VPC Peering / Secret Manager
# -----------------------------------------------------------------------------
# demo の cloudsql モジュール相当。構造化結果を保存するリレーショナルDB。
# このモジュールは「単体で完結」する。VPC も自分で作る（01-network には依存しない）。
#   → 01-network と名前が衝突しないよう、VPC 名は ${env}-${vpc_name}（demo-sql）にする。
#
# ファイル構成:
