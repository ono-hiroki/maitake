...
07 本体は GCS 源で、「GCS サービスエージェントに pubsub.publisher 付与」が必要だった。
ここを Pub/Sub 源にするとその一手間が消え、Eventarc の核（SA・サービスエージェント・
トリガー・伝播待ち）だけに集中できる。
...
- **`matching_criteria` の type** で「どのイベントか」を指定:
  ...
  - `google.cloud.storage.object.v1.finalized`（07 本体の GCS）
...
- これに「GCS 源」と「起動先を Cloud Run Job」を足すと 07-workflow 本体になる。
