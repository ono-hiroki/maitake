# cloudsql — Cloud SQL / VPC Peering / Secret Manager

処理結果を保存するリレーショナルDB（PostgreSQL）。
**このモジュールは単体で完結する**（VPC も自分で作るので vpc-network には依存しない）。

## 前提

- なし（VPC を含め必要なものは全てこのサンプルで作成する）
- `vpc-network` サンプルと同時に作っても名前が衝突しないよう、VPC 名は `sbx-demo-sql`
