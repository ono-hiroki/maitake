...
`./manifests/nginxserver-replicaset.yaml`:
...
```bash
k apply -f ./manifests/nginxserver-replicaset.yaml
```
...
```bash
k apply -f ./manifests/nginxserver-service.yaml
```
...
### Service タイプが ClusterIP であることを確認

Ingress のバックエンドには ClusterIP で十分です。NodePort や LoadBalancer は不要です。

```bash
# dev namespace の Service 一覧（タイプを確認）
k get service -n dev -o custom-columns=NAME:.metadata.name,TYPE:.spec.type
```
...
- `TYPE` が `ClusterIP` になっている → ○
- `NodePort` や `LoadBalancer` ではない → ○（不要なポート開放やLB作成がない）
...
`./manifests/nginxserver-ingress.yaml`:
...
```bash
k apply -f ./manifests/nginxserver-ingress.yaml
```
...
nginx.conf を確認します。ファイルが長いので、`nginxserver-service` で検索します。
...
| 変数 | 値 | 説明 |
|------|-----|------|
| `$namespace` | `dev` | Ingress の namespace |
| `$ingress_name` | `nginxserver-ingress` | Ingress リソース名 |
| `$service_name` | `nginxserver-service` | バックエンドの Service 名 |
| `$location_path` | `/` | ルーティングするパス |
...
> **Note**: `localhost:10246` は NGINX Ingress Controller が提供する内部 API です。
> nginx 自体の機能ではなく、Controller（Go 製）が追加で提供しています。
> 現在の設定状態を JSON で確認でき、デバッグや運用時に便利です。

```bash
k exec -n ingress-nginx $INGRESS_POD -- curl -s localhost:10246/configuration/backends
```
...
```bash
k delete -f ./manifests/nginxserver-ingress.yaml
```
...
> **Note**: 本来は異なる Service にルーティングしますが、ここでは既存の `nginxserver-service` を使って複数パスの設定方法を学びます。
...
```bash
k apply -f ./manifests/multi-path-ingress.yaml
```
...
> **Note**: 実際の運用では `/app` と `/web` で異なる Service を指定することで、マイクロサービスのルーティングを実現します。
...
k delete -f ./manifests/nginxserver-replicaset.yaml
```

> **Note**: `nginxserver-ingress.yaml` は複数パスのセクションで既に削除済みです。
