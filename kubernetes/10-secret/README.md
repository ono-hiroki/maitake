...
```bash
k apply -f manifests/secret.yaml
```
...
**注意**: `describe` では値が表示されません（バイト数のみ）。
...
```bash
k apply -f manifests/pod-env.yaml
```
...
```bash
k delete -f manifests/pod-env.yaml
```
...
```bash
k apply -f manifests/pod-volume.yaml
```
...
```bash
k delete -f manifests/pod-volume.yaml
```
...
```bash
k delete -f manifests/secret.yaml
```
