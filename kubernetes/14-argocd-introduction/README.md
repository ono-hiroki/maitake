...
Argo CD は、Kubernetes 向けの宣言的な GitOps 継続的デリバリーツールです。Git リポジトリをソースオブトゥルース（信頼できる唯一の情報源）として、アプリケーションの定義・設定・環境を管理します。
...
- マニフェスト管理ツール（Helm, Kustomize）の基礎
  - 参照: [Helmでパッケージ管理する](../11-helm/README.md)
  - 参照: [Kustomizeで環境別マニフェストを管理する](../12-kustomize/README.md)
...
# 作業ディレクトリに移動
cd kubernetes/14-argocd-introduction
```
...
```bash
k apply -f examples/guestbook-app.yaml
```
